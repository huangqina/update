...
int run(string ip,string cmd) //run test
{
	...
	{
		//predefine some expected responses
		result = exp_expectl(fd, exp_glob, "*assword*", 1, exp_exact, "Permission denied, please try again.", 2, exp_regexp, "(The authenticity of host)(.)*(Are you sure you want to continue connecting (yes/no)?)", 3, exp_glob, "*sudo*", 4,exp_glob, "*assphrase: ", 5,exp_glob, "*elect*: ", 6,exp_glob, "*Running*: ", 7, exp_end);
		//char pas[] = "123456\n";
		switch(result)
		{
			case 1:
		...
		for (int i = 0; i<sizeof(ip);i++){
            for (int i = 0; i<filename.size();i++){
                if(filename[i] == "DB.zip"){
                    string cmd = "";
                    mount("/dev/sda4",ip[i]);
                    cmd += "sudo -S mv -f /mnt/DB /mnt/DB3;";
                    cmd += "sudo -S mv -f /mnt/DB2 /mnt/DB;";
                    cmd += "sudo -S mv -f /mnt/DB3 /mnt/DB2;";
                    cmd += "sudo -S cp -f /mnt/DB ~/";
                    run(ip[i],cmd);
                    umount(ip[i]);
                }
